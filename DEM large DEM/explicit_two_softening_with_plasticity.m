...
E_T_2 = (phi^2/(1-phi))*(Element_Length.^2*G/t^2)/((G/tau_s)*(gamma_p_s-gamma_p_s1) + 1);
E_T_3 = (phi^2/(1-phi))*(Element_Length.^2*G/t^2)/((G/tau_s1)*(gamma_p_s1-gamma_u) + 1);
H_1 = E.*E_T_1./(E-E_T_1);
H_2 = E.*E_T_2./(E-E_T_2);
H_3 = E.*E_T_3./(E-E_T_3);
sigma_y_1 = tau_y*phi*Element_Length/t;
sigma_y_2 = tau_s*phi*Element_Length/t;
epsilon_p_u = (1./(Element_Length/t))*((1-phi)/phi)*(gamma_p_s);
epsilon_p_u1 = (1./(Element_Length/t))*((1-phi)/phi)*(gamma_p_s1);
...
while i<n+1  
   ...
   % while conv > tol && iter < max_iter 
       ...
       % Delta_u(:,1) = Delta_u(:,1) + delta_u(:,1);
        ...
        I3 = find(Plastic_Strain(:,i-1)>=epsilon_p_u & Plastic_Strain(:,i-1)<epsilon_p_u1);  %softening 1
%         mm= find(Plastic_Strain(:,i-1)<epsilon_p_u1);  %softening 1
%         if ll==zeros(0,1)
%             I3=ll;
%         else
%            I3 = find(ll<=epsilon_p_u1);                    %softening 1
%         end
%         I3 = ll+ mm;    
        
        I4 = find(Plastic_Strain(:,i-1)>=epsilon_p_u1) %softening 2
        zz = setdiff([1:p.Num_Elements]', I3);
        I8 = setdiff(zz, I4);
...
%         end
        ...
        tr_sigma(I4,1) = Stress(I4,i-1) + E(I4).*Delta_Strain(I4,1);
        ...
        sigma_yeild(I4,i) = sigma_y_2(I4) + H_2(I4).*epsilon_p_u1(I4) + H_3(I4).*(Plastic_Strain(I4,i-1)-epsilon_p_u1(I4));
        ...
        Delta_plastic_strain(I4,1) = fr(I4)./(E(I4) + H_3(I4));
        ...
        p.Element_E(I4) = E_T_3(I4);
        
        % Storing Colour information for Stress-Strain Curve
        I6 = I0;
        I9 = find(Plastic_Strain(:,i)>0);
        Common_Elements = intersect(I6,I9);
