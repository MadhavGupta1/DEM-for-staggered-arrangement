...
function Displacements = Displacement(p,Stiffness)
...
p.Force_Modified=p.Force(active_Dof)-Stiffness(active_Dof,p.Prescribed_Dof)*p.Displacement(p.Prescribed_Dof);

%Solution U = Inverse of modified[K] x F for Active Dof
U=Stiffness(active_Dof,active_Dof)\p.Force_Modified;  
