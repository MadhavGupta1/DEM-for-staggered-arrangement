...
E_T_1 = (phi^2/(1-phi))*(Element_Length.^2*G/t^2)/((G/tau_s)*(gamma_p_s-gamma_u) + 1);
% H_1 = E.*E_T_1./(E-E_T_1);
% H_2 = E.*E_T_2./(E-E_T_2);
H_1 = E.*E_T_1./(E-E_T_1);
...
fr = abs(tr_sigma(I8,1))-sigma_yeild(I8,i); %Finding postion of current point(elastic/plastic)
I0 = find(fr<=0);   %These are in elastic region
I1 = setdiff([1:p.Num_Elements]', I0); %These are in plastic region
...
yx = size(Stress_combined_1);
yy= yx(1,2);
Friction_stress=zeros(1,yy);
for m=1:yy-1
if Stress_combined_1(1,m+1) > Stress_combined_1(1,m)
    continue
else
    Friction_stress(1,m) = (Stress_combined_1(1,m))/5;
end
end
Stress_combined_11=zeros(1,yy);
for m=1:yy-1
if Stress_combined_1(1,m+1) < Stress_combined_1(1,m)
    continue
else
    Stress_combined_11(1,m) = (Stress_combined_1(1,m));
end
end
Stress_with_friction = Stress_combined_11 + Friction_stress;
figure(1)
plot(Strain_combined_1,Stress_with_friction,'k','LineWidth',1.5)
hold on
axis([0 inf 0 inf])
xlabel('Strain');
ylabel('Stress');
